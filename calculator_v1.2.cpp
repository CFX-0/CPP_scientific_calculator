...
int main(){
  ...
  cin >> x;
  cout << "Please enter the operation you want to perform /, x, +, -, c(cubic root), s(square root), !(factorial, cap at 25), ^(power [a of b/ c of d]): ";
  cin >> operation;
  if (operation){
    if (operation == '^'){
      y = 1;
  ...
  cin >> credits;
  if(credits == 'y'){
    cout << "-CALCULATOR-© v1.2 by liquidfy_"<<endl;
  }
    else{
